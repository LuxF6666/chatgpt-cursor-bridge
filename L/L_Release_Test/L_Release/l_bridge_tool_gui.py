...
class BridgeGUI(QtWidgets.QWidget):
    ...
    def capture_template(self, template_type):
        ...
        if rect.width() < 10 or rect.height() < 10:
            self.log(f"选择区域太小，取消: {template_name}")
            return

        x1 = max(0, rect.left())
        y1 = max(0, rect.top())
        x2 = min(screenshot.shape[1], rect.right())
        y2 = min(screenshot.shape[0], rect.bottom())

        cropped = screenshot[y1:y2, x1:x2]

        template_path = self.get_template_path(template_name)
        if not template_path:
            self.log(f"无法获取模板路径: {template_name}")
            self.set_error("模板路径获取失败")
            return

        try:
            success = cv2.imwrite(str(template_path), cropped)
            if success:
                self.log(f"已保存模板: {template_name} ({rect.width()}x{rect.height()})")
            else:
                self.log(f"保存模板失败: {template_name}")
                self.set_error(f"保存模板失败: {template_name}")
        except Exception as e:
            self.log(f"保存模板异常: {str(e)}")
            self.set_error(f"保存模板异常: {str(e)}")
    ...
    def run_system_check(self):
        ...
        try:
            screenshot = pyautogui.screenshot()
            self.log(f"✓ 屏幕截图: 成功 ({screenshot.size})")
            self.diagnostic_data.append(f"屏幕截图: 成功 ({screenshot.size})")
            checks.append(True)
        except Exception as e:
            self.log(f"✗ 屏幕截图: 失败 - {str(e)}")
            self.diagnostic_data.append(f"屏幕截图: 失败 - {str(e)}")
