...
def load_qpixmap_unicode(path, max_size=None):
    try:
        ...
        print(f"load_qpixmap_unicode 失败: {str(e)}")
        return None
            ...
            try:
                ...
                if template_path and template_path.exists():
                    ...
                    if test_img is not None:
                        h, w = test_img.shape[:2]
                        self.log(f"✓ 模板 {template}: 已采集，文件大小: {file_size} bytes，读取尺寸: {w}x{h}")
                        template_status.append(f"{template}: 已采集，读取尺寸: {w}x{h}")
                    else:
                        self.log(f"✗ 模板 {template}: 文件存在但读取失败，可能是中文路径或图片损坏: {template_path}")
                        template_status.append(f"{template}: 文件存在但读取失败")
                        issues.append(f"模板 {template} 存在但读取失败")
                else:
